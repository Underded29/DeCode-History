// src/components/MythDetail/MythQuiz.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';

// 1. Прибрали імпорт mockMyths
// 2. Додали totalMyths у деструктуризацію пропсів
const MythQuiz = ({ quizzes, currentId, totalMyths }) => {
  ...
  // 3. Видалили рядок const totalMyths = mockMyths.length; бо тепер це пропс
  ...
  const handleNextMyth = () => {
    if (currentIdNum < totalMyths) navigate(`/myth/${currentIdNum + 1}`);
  };

  const handlePrevMyth = () => {
    if (currentIdNum > 1) navigate(`/myth/${currentIdNum - 1}`);
  };
    ...
    <div className="w-full bg-brand-blue/5 border border-brand-blue/20 rounded-3xl p-6 md:p-8 mt-4 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {options.map((option) => {
          ...
          if (selectedAnswer) {
              btnClass += "cursor-default "; // Забираємо pointer, якщо вже відповіли
          } else {
          ...
          else if (selectedAnswer) btnClass += "border-brand-blue/10 opacity-60"; // Робимо неактивні блідими
      ...
      {/* Пояснення з'являється після відповіді */}
      {selectedAnswer && (
      ...
      {/* Динамічна навігація внизу */}
      <div className="flex justify-between items-center pt-4 mt-2 border-t border-brand-blue/20">
        <button 
            onClick={handlePrevMyth}
            disabled={currentIdNum === 1}
            className={`font-bold px-4 py-2 rounded-xl transition-colors flex items-center gap-2 ${currentIdNum === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-brand-blue hover:bg-brand-blue/10'}`}
        >
          ← Попередній міф
        </button>

        {/* Логіка кнопок: чекаємо на відповідь, потім "Наступне питання", в кінці - "Наступний міф" */}
        {selectedAnswer ? (
            isLastQuestion ? (
                <button 
                    onClick={handleNextMyth}
                    disabled={currentIdNum === totalMyths}
                    className={`font-bold px-6 py-2.5 rounded-xl transition-colors flex items-center gap-2 shadow-md ${currentIdNum === totalMyths ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-brand-blue hover:bg-brand-blue/90 text-white'}`}
                >
                    Наступний міф →
                </button>
            ) : (
