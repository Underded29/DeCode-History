...
const Register = () => {
  ...
  const handleChange = (e) => {
    ...
    // Очищаємо серверну помилку при введенні нових даних
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    setServerError(''); // Очищаємо попередні помилки

    // Формуємо дані для відправки на сервер
    const userData = {
      username: formData.name, // Наш бекенд очікує поле username
    ...
    try {
      ...
      if (response.ok) {
        // Успішно! Зберігаємо токен і дані юзера
        ...
        // Якщо бекенд повернув помилку (400)
        setServerError(data.error || 'Сталася помилка під час реєстрації');
      }
    } catch (error) {
